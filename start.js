...
function drawStart() {
  ...
  text("The ION LRT has gone out of control and is speeding toward a car stuck on the track. \n You are standing at the lever that controls the track switch.\n Do You:", width / 2, 180);
  ...
  const ChoiceBtnTwo = {
    ...
    w: 600,
    h: 80,
    label: "Pull the lever, possibly causing derailment?!?!",
  };

  // Draw both buttons
  drawButton(ChoiceBtnOne);
  drawButton(ChoiceBtnTwo);

  // ---- Cursor feedback ----
  // If the mouse is over either button, show a hand cursor
  // so the player knows it is clickable.
  const over = isHover(ChoiceBtnOne) || isHover(ChoiceBtnTwo);
...
function startMousePressed() {
  ...
  const ChoiceBtnOne = { x: width / 2, y: 320, w: 400, h: 80 };
  const ChoiceBtnTwo = { x: width / 2, y: 430, w: 600, h: 80 };

  // If START is clicked, go to the game screen
  if (isHover(ChoiceBtnOne)) {
    currentScreen = "DecisionOneA";
  }
  // If INSTRUCTIONS is clicked, go to the instructions screen
  else if (isHover(ChoiceBtnTwo)) {
