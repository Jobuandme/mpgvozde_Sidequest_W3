...
// ------------------------------------------------------------
// drawDecOneA() is called from main.js only when:
// currentScreen === "DecisionOneA"
function drawDecOneA() {
  // Background colour for the start screen
  background(180, 125, 220); // soft teal background

  // ---- Title text ----
  fill(30, 50, 60);
  textSize(46);
  textAlign(CENTER, CENTER);
  text("You let the ION hit the Car!!!\n\n", width / 2, 180);
  textSize(23);
  textAlign(CENTER, CENTER);
  text("Everyone in the ION is fine,\n but the car is destroyed and the driver has been injured.", width / 2, 180);
  ...
  const startBtn = {
    ...
    w: 400,
    h: 80,
    label: "You feel guilty for not trying \nto switch the lever",
  };
  ...
  const instrBtn = {
    ...
    w: 650,
    h: 80,
    label: "You dont feel guilty, the car getting \nstuck wasnt your fault, and you're still a bystander",
  };
...
// ------------------------------------------------------------
// Called from main.js only when currentScreen === "DecisionOneA"
function DecisionOneAMousePressed() {
  // For input checks, we only need x,y,w,h (label is optional)
  const startBtn = { x: width / 2, y: 320, w: 400, h: 80 };
  const instrBtn = { x: width / 2, y: 430, w: 650, h: 80 };

  // If START is clicked, go to the game screen
  if (isHover(startBtn)) {
    currentScreen = "EndingA";
  }
